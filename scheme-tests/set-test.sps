...
(define test-equal
  (case-lambda
    ((x y)
     (test-equal "" x y))
    ((name x y)
     (test-assert name ((current-test-comparator) x y)))))

(test-begin "scheme-set")
...
(test-group "sets"
            ...
            (test-group "sets/subsets"
                        (let ((set2 (set number-comparator 1 2))
                              ...
                              (setx (set number-comparator 10 20 30 40)))
