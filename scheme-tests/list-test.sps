...
(for-each (lambda (exp act)
            (test-approximate exp act 0.0001))
          '(0 -0.1 -0.2 -0.3 -0.4)
...
(test-assert (not (list= char-ci=? '(#\a #\B) '(#\a))))
(test-assert (list= char-ci=? '(#\a #\B) '(#\A #\b)))
(test-assert (not (list= char-ci=? '(#\a #\B) '(#\a) '(#\A #\B))))
(test-assert (list= char-ci=? '(#\a #\B) '(#\A #\b) '(#\A #\B)))
...
(let-values (((a b) (split-at '(a b c d e f g h) 3)))
            (test-equal '(a b c) a)
            (test-equal '(d e f g h) b))
...
(test-equal '((one 1 odd) (two 2 even) (three 3 odd)) (zip '(one two three) '(1 2 3) '(odd even odd even odd even odd even)))
(test-equal '((1) (2) (3)) (zip '(1 2 3)))
(test-equal '((3 #f) (1 #t) (4 #f) (1 #t)) (zip '(3 1 4 1) (circular-list #f #t)))
(let-values (((a b) (unzip2 '((1 one) (2 two) (3 three)))))
            (test-equal '(1 2 3) a)
            (test-equal '(one two three) b))
...
(test-equal '((a b c) (1 2 3) (b c) (2 3) (c) (3)) (pair-fold-right cons* '() '(a b c) '(1 2 3)))
...
(test-equal '#(0 1 4 9 16) (let ((v (make-vector 5))) (for-each (lambda (i) (vector-set! v i (* i i))) '(0 1 2 3 4)) v))
(test-equal '(1 -1 3 -3 8 -8) (append-map (lambda (x) (list x (- x))) '(1 3 8)))
(test-equal '(1 -1 3 -3 8 -8) (apply append (map (lambda (x) (list x (- x))) '(1 3 8))))
(test-equal '(1 -1 3 -3 8 -8) (append-map! (lambda (x) (list x (- x))) '(1 3 8)))
(test-equal '(1 -1 3 -3 8 -8) (apply append! (map (lambda (x) (list x (- x))) '(1 3 8))))
(test-equal "pair-for-each-1" '((a b c) (b c) (c))
      (let ((a '()))
        (pair-for-each (lambda (x) (set! a (cons x a))) '(a b c))
        (reverse a)))
(test-equal '(1 9 49) (filter-map (lambda (x) (and (number? x) (* x x))) '(a 1 b 3 c 7)))
(test-equal '(0 8 8 -4) (filter even? '(0 7 8 8 43 -4)))
(let-values (((a b) (partition symbol? '(one 2 3 four five 6))))
            (test-equal '(one four five) a)
            (test-equal '(2 3 6) b))
...
;(test-equal 'error/undefined (find even? '(1 2 . x)))
;(test-equal 'error/undefined (any  even? '(1 2 . x))) ; success, error or other
...
(let-values (((a b) (span even? '(2 18 3 10 22 9))))
            (test-equal '(2 18) a)
            (test-equal '(3 10 22 9) b))
(let-values (((a b) (break even? '(3 1 4 1 5 9))))
  (test-equal '(3 1) a)
  (test-equal '(4 1 5 9) b))
...
;(test-equal '*unspecified* (memq 101 '(100 101 102)))
(test-equal '(101 102) (memv 101 '(100 101 102)))
(test-equal '(a b c z) (delete-duplicates '(a b a c a b c z)))
(test-equal '((a . 3) (b . 7) (c . 1)) (delete-duplicates '((a . 3) (b . 7) (a . 9) (c . 1)) (lambda (x y) (eq? (car x) (car y)))))
(let ((e '((a 1) (b 2) (c 3))))
  ...
  ;(test-equal '*unspecified* (assq 5 '((2 3) (5 7) (11 13))))
...
;; This does not hold in Gauche - for constant list, see ilist?
;(let ((f (lambda () (list 'not-a-constant-list)))
;      (g (lambda () '(constant-list))))
  ;(test-equal '*unspecified* (set-car! (f) 3))
