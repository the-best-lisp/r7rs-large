...
(test-assert (eqv? #t (textual-every (lambda (c) (if (char? c) c #f))
                                     (text))))

(test-assert (eqv? #\c (textual-every (lambda (c) (if (char? c) c #f))
                                      (as-text "abc"))))

(test-assert (eqv? #f (textual-every (lambda (c) (if (char>? c #\b) c #f))
                                     (as-text "abc"))))

(test-assert (eqv? #\c (textual-every (lambda (c) (if (char>? c #\b) c #f))
                                      (as-text "abc") 2)))

(test-assert (eqv? #t (textual-every (lambda (c) (if (char>? c #\b) c #f))
                                     (as-text "abc") 1 1)))

(test-assert (eqv? #f (textual-any (lambda (c) (if (char? c) c #f))
                                   (text))))

(test-assert (eqv? #\a (textual-any (lambda (c) (if (char? c) c #f))
                                    (as-text "abc"))))

(test-assert (eqv? #\c (textual-any (lambda (c) (if (char>? c #\b) c #f))
                                    (as-text "abc"))))

(test-assert (eqv? #\c (textual-any (lambda (c) (if (char>? c #\b) c #f))
                                    (as-text "abc") 2)))

(test-assert (eqv? #f (textual-any (lambda (c) (if (char>? c #\b) c #f))
                                   (as-text "abc") 0 2)))


(test-assert (eqv? #t (textual-every (lambda (c) (if (char? c) c #f)) "")))

(test-assert (eqv? #\c (textual-every (lambda (c) (if (char? c) c #f)) "abc")))

(test-assert (eqv? #f (textual-every (lambda (c) (if (char>? c #\b) c #f)) "abc")))

(test-assert (eqv? #\c (textual-every (lambda (c) (if (char>? c #\b) c #f)) "abc" 2)))

(test-assert (eqv? #t (textual-every (lambda (c) (if (char>? c #\b) c #f)) "abc" 1 1)))

(test-assert (eqv? #f (textual-any (lambda (c) (if (char? c) c #f)) "")))

(test-assert (eqv? #\a (textual-any (lambda (c) (if (char? c) c #f)) "abc")))

(test-assert (eqv? #\c (textual-any (lambda (c) (if (char>? c #\b) c #f)) "abc")))

(test-assert (eqv? #\c (textual-any (lambda (c) (if (char>? c #\b) c #f)) "abc" 2)))

(test-assert (eqv? #f (textual-any (lambda (c) (if (char>? c #\b) c #f)) "abc" 0 2)))
...
(test-assert (result=? "" (text-unfold null? car cdr '())))

(test-assert (result=? "abc"
                       (text-unfold null? car cdr (string->list "abc"))))

(test-assert (result=? "def"
                       (text-unfold null? car cdr '() (string->text "def"))))

(test-assert (result=? "defabcG"
                       (text-unfold null?
                                    car
                                    cdr
                                    (string->list "abc")
...
(test-assert (result=? "" (text-unfold-right null? car cdr '())))

(test-assert (result=? "cba"
                       (text-unfold-right null? car cdr (string->list "abc"))))

(test-assert (result=? "def"
                       (text-unfold-right null? car cdr '() (string->text "def"))))

(test-assert (result=? "Gcbadef"
...
(test-assert (result=? (string-append "%="
                                      (make-string 200 #\*)
...
(test-assert (result=? " The English alphabet: abcdefghijklmnopqrstuvwxyz "
...
(test-assert (let ((txt (textual->text "str" "not a textual")))
               (and (text? txt)
                    (textual=? txt "str"))))

(test-assert (let ((txt (textual->text (text #\s #\t #\r) "bad textual")))
               (and (text? txt)
                    (textual=? txt "str"))))
...
(test-assert (equal? '#() (textual->vector (text))))

(test-assert (equal? '#() (textual->vector (text) 0)))

(test-assert (equal? '#() (textual->vector (text) 0 0)))

(test-assert (equal? '#(#\a #\b #\c) (textual->vector (text #\a #\b #\c))))

(test-assert (equal? '#() (textual->vector (text #\a #\b #\c) 3)))

(test-assert (equal? '#(#\b #\c) (textual->vector (text #\a #\b #\c) 1 3)))


(test-assert (equal? '#() (textual->vector "")))

(test-assert (equal? '#() (textual->vector "" 0)))

(test-assert (equal? '#() (textual->vector "" 0 0)))

(test-assert (equal? '#(#\a #\b #\c) (textual->vector "abc")))

(test-assert (equal? '#() (textual->vector "abc" 3)))

(test-assert (equal? '#(#\b #\c) (textual->vector "abc" 1 3)))


(test-assert (equal? '() (textual->list (text))))

(test-assert (equal? '() (textual->list (text) 0)))

(test-assert (equal? '() (textual->list (text) 0 0)))

(test-assert (equal? '(#\a #\b #\c) (textual->list (text #\a #\b #\c))))

(test-assert (equal? '() (textual->list (text #\a #\b #\c) 3)))

(test-assert (equal? '(#\b #\c) (textual->list (text #\a #\b #\c) 1 3)))


(test-assert (equal? '() (textual->list "")))

(test-assert (equal? '() (textual->list "" 0)))

(test-assert (equal? '() (textual->list "" 0 0)))

(test-assert (equal? '(#\a #\b #\c) (textual->list "abc")))

(test-assert (equal? '() (textual->list "abc" 3)))

(test-assert (equal? '(#\b #\c) (textual->list "abc" 1 3)))
...
(test-assert (equal? '#u8(254 255 0 97 0 98 0 99)
                     (textual->utf16 (as-text "abc"))))

(test-assert (equal? '#u8(254 255 0 97 0 98 0 99)
                     (textual->utf16 "abc")))

(test-assert (equal? '#u8(254 255 0 97 0 98 0 99 0 121 0 121 0 121 0 122 0 122 0 122)
...
(test-assert (equal? '#u8(0 97 0 98 0 99)
                     (textual->utf16be (as-text "abc"))))

(test-assert (equal? '#u8(0 97 0 98 0 99)
                     (textual->utf16be "abc")))

(test-assert (equal? '#u8(0 97 0 98 0 99 0 121 0 121 0 121 0 122 0 122 0 122)
...
(test-assert (equal? '#u8(97 0 98 0 99 0)
                     (textual->utf16le (as-text "abc"))))

(test-assert (equal? '#u8(97 0 98 0 99 0)
                     (textual->utf16le "abc")))

(test-assert (equal? '#u8(97 0 98 0 99 0 121 0 121 0 121 0 122 0 122 0 122 0)
...
(test-assert (result=? "It's lots of fun to code it up in Scheme."
                       (textual-replace (as-text "It's easy to code it up in Scheme.")
                                        (as-text "lots of fun")
                                        5 9)))

(test-assert (result=? "The miserable perl programmer endured daily ridicule."
                       (textual-replace "The TCL programmer endured daily ridicule."
                                        (as-text "another miserable perl drone")
                                        4 7 8 22)))

(test-assert (result=? "It's really easy to code it up in Scheme."
                       (textual-replace (as-text "It's easy to code it up in Scheme.")
                                        "really "
                                        5 5)))

(test-assert (result=? "Runs in O(1) time." ; for texts (using sample implementations)
...
(test-assert (equal? #t (textual=? (as-text "Strasse") (as-text "Strasse"))))

(test-assert (equal? #t (textual=? "Strasse" (as-text "Strasse") "Strasse")))

(test-assert (equal? #f (textual<? (as-text "z") (as-text "z"))))
(test-assert (equal? #t (textual<? (as-text "z") "zz")))
(test-assert (equal? #f (textual<? (as-text "z") (as-text "Z"))))
(test-assert (equal? #t (textual<=? (as-text "z") "zz")))
(test-assert (equal? #f (textual<=? "z" "Z")))
(test-assert (equal? #t (textual<=? "z" (as-text "z"))))

(test-assert (equal? #f (textual<? "z" (as-text "z"))))
(test-assert (equal? #f (textual>? (as-text "z") "zz")))
(test-assert (equal? #t (textual>? "z" (as-text "Z"))))
(test-assert (equal? #f (textual>=? (as-text "z") "zz")))
(test-assert (equal? #t (textual>=? "z" "Z")))
(test-assert (equal? #t (textual>=? (as-text "z") (as-text "z"))))
...
(let* ((w "a")
  ...
  (test-assert (equal? (textual=? x y z)                           #f))
  (test-assert (equal? (textual=? x x z)                           #t))
  (test-assert (equal? (textual=? w x y)                           #f))
  (test-assert (equal? (textual=? y x w)                           #f))

  (test-assert (equal? (textual<? x y z)                           #f))
  (test-assert (equal? (textual<? x x z)                           #f))
  (test-assert (equal? (textual<? w x y)                           #t))
  (test-assert (equal? (textual<? y x w)                           #f))

  (test-assert (equal? (textual>? x y z)                           #f))
  (test-assert (equal? (textual>? x x z)                           #f))
  (test-assert (equal? (textual>? w x y)                           #f))
  (test-assert (equal? (textual>? y x w)                           #t))
  ...
  (test-assert (equal? (textual=? x x)                             #t))
  (test-assert (equal? (textual=? w x)                             #f))
  (test-assert (equal? (textual=? y x)                             #f))

  (test-assert (equal? (textual<? x x)                             #f))
  (test-assert (equal? (textual<? w x)                             #t))
  (test-assert (equal? (textual<? y x)                             #f))

  (test-assert (equal? (textual>? x x)                             #f))
  (test-assert (equal? (textual>? w x)                             #f))
  (test-assert (equal? (textual>? y x)                             #t))
...
(test-assert (equal? #t (textual-ci<? "a" "Z")))
(test-assert (equal? #t (textual-ci<? "A" "z")))
(test-assert (equal? #f (textual-ci<? "Z" "a")))
(test-assert (equal? #f (textual-ci<? "z" "A")))
(test-assert (equal? #f (textual-ci<? "z" "Z")))
(test-assert (equal? #f (textual-ci<? "Z" "z")))
(test-assert (equal? #f (textual-ci>? "a" "Z")))
(test-assert (equal? #f (textual-ci>? "A" "z")))
(test-assert (equal? #t (textual-ci>? "Z" "a")))
(test-assert (equal? #t (textual-ci>? "z" "A")))
(test-assert (equal? #f (textual-ci>? "z" "Z")))
(test-assert (equal? #f (textual-ci>? "Z" "z")))
(test-assert (equal? #t (textual-ci=? "z" "Z")))
(test-assert (equal? #f (textual-ci=? "z" "a")))
(test-assert (equal? #t (textual-ci<=? "a" "Z")))
(test-assert (equal? #t (textual-ci<=? "A" "z")))
(test-assert (equal? #f (textual-ci<=? "Z" "a")))
...
(cond-expand
  (full-unicode-strings
    (test-assert (equal? #f (textual=? ABCDEF DEFABC)))
    (test-assert (equal? #f (textual=? DEFABC ABCDEF)))
    (test-assert (equal? #t (textual=? DEFABC DEFABC)))

    (test-assert (equal? #f (textual<? ABCDEF DEFABC)))
    (test-assert (equal? #t (textual<? DEFABC ABCDEF)))
    (test-assert (equal? #f (textual<? DEFABC DEFABC)))

    (test-assert (equal? #t (textual>? ABCDEF DEFABC)))
    (test-assert (equal? #f (textual>? DEFABC ABCDEF)))
    (test-assert (equal? #f (textual>? DEFABC DEFABC)))
    ...
    (test-assert (equal? #f (textual=? "Fuss" fuss)))
    (test-assert (equal? #f (textual=? "Fuss" "Fuss" fuss)))
    (test-assert (equal? #f (textual=? "Fuss" fuss "Fuss")))
    (test-assert (equal? #f (textual=? fuss "Fuss" "Fuss")))
    (test-assert (equal? #t (textual<? "z" (as-text eszett))))
    (test-assert (equal? #f (textual<? (as-text eszett) "z")))
    (test-assert (equal? #t (textual<=? "z" (as-text eszett))))
    (test-assert (equal? #f (textual<=? (as-text eszett) "z")))
    (test-assert (equal? #f (textual>? "z" (as-text eszett))))
    (test-assert (equal? #t (textual>? (as-text eszett) "z")))
    (test-assert (equal? #f (textual>=? "z" (as-text eszett))))
    (test-assert (equal? #t (textual>=? (as-text eszett) "z")))
...
(test-assert (= 0 (textual-prefix-length ABC ABCDEF)))

(test-assert (= 0 (textual-prefix-length ABCDEF ABC)))

(test-assert (= 0 (textual-prefix-length ABCDEF DEFABC)))

(test-assert (= 6 (textual-prefix-length DEFABC DEFABC)))

(test-assert (= 6 (textual-prefix-length (textual->string DEFABC) DEFABC)))

(test-assert (= 6 (textual-prefix-length DEFABC (textual->string DEFABC))))

(test-assert (= 6 (textual-prefix-length (textual->string DEFABC)
                                         (textual->string DEFABC))))

(test-assert (= 0 (textual-prefix-length (as-text "") (as-text ""))))

(test-assert (= 0 (textual-prefix-length (as-text "") (as-text "aabbccddee"))))

(test-assert (= 0 (textual-prefix-length (as-text "aisle") (as-text ""))))

(test-assert (= 0 (textual-prefix-length (as-text "") (as-text "aabbccddee"))))

(test-assert (= 1 (textual-prefix-length (as-text "aisle") (as-text "aabbccddee"))))

(test-assert (= 0 (textual-prefix-length (as-text "bail") (as-text "aabbccddee"))))

(test-assert (= 4 (textual-prefix-length (as-text "prefix") (as-text "preface"))))

(test-assert (= 0 (textual-prefix-length (as-text "") (as-text "") 0)))

(test-assert (= 0 (textual-prefix-length (as-text "") (as-text "aabbccddee") 0)))

(test-assert (= 0 (textual-prefix-length (as-text "aisle") (as-text "") 0)))

(test-assert (= 1 (textual-prefix-length (as-text "aisle") (as-text "aabbccddee") 0)))

(test-assert (= 0 (textual-prefix-length (as-text "bail") (as-text "aabbccddee") 0)))

(test-assert (= 4 (textual-prefix-length (as-text "prefix") (as-text "preface") 0)))

(test-assert (= 0 (textual-prefix-length (as-text "aisle") (as-text "") 1)))

(test-assert (= 0 (textual-prefix-length (as-text "aisle") (as-text "aabbccddee") 1)))

(test-assert (= 1 (textual-prefix-length (as-text "bail") (as-text "aabbccddee") 1)))

(test-assert (= 0 (textual-prefix-length (as-text "prefix") (as-text "preface") 1)))

(test-assert (= 0 (textual-prefix-length (as-text "") (as-text "") 0 0)))

(test-assert (= 0 (textual-prefix-length (as-text "") (as-text "aabbccddee") 0 0)))

(test-assert (= 0 (textual-prefix-length (as-text "aisle") (as-text "") 0 4)))

(test-assert (= 1 (textual-prefix-length (as-text "aisle") (as-text "aabbccddee") 0 4)))

(test-assert (= 0 (textual-prefix-length (as-text "bail") (as-text "aabbccddee") 0 1)))

(test-assert (= 0 (textual-prefix-length (as-text "aisle") (as-text "") 1 4)))

(test-assert (= 0 (textual-prefix-length (as-text "aisle") (as-text "aabbccddee") 1 4)))

(test-assert (= 1 (textual-prefix-length (as-text "bail") (as-text "aabbccddee") 1 4)))

(test-assert (= 0 (textual-prefix-length (as-text "prefix") (as-text "preface") 1 5)))

(test-assert (= 0 (textual-prefix-length (as-text "") (as-text "") 0 0 0)))

(test-assert (= 0 (textual-prefix-length (as-text "") (as-text "aabbccddee") 0 0 0)))

(test-assert (= 0 (textual-prefix-length (as-text "aisle") (as-text "") 0 4 0)))

(test-assert (= 0 (textual-prefix-length (as-text "aisle") (as-text "aabbccddee") 0 4 2)))

(test-assert (= 1 (textual-prefix-length (as-text "bail") (as-text "aabbccddee") 0 1 2)))

(test-assert (= 0 (textual-prefix-length (as-text "prefix") (as-text "preface") 0 5 1)))

(test-assert (= 0 (textual-prefix-length (as-text "aisle") (as-text "") 1 4 0)))

(test-assert (= 0 (textual-prefix-length (as-text "aisle") (as-text "aabbccddee") 1 4 3)))

(test-assert (= 0 (textual-prefix-length (as-text "bail") (as-text "aabbccddee") 1 4 3)))

(test-assert (= 3 (textual-prefix-length (as-text "prefix") (as-text "preface") 1 5 1)))

(test-assert (= 0 (textual-prefix-length (as-text "") (as-text "") 0 0 0 0)))

(test-assert (= 0 (textual-prefix-length (as-text "") (as-text "aabbccddee") 0 0 0 0)))

(test-assert (= 0 (textual-prefix-length (as-text "aisle") (as-text "") 0 4 0 0)))

(test-assert (= 0 (textual-prefix-length (as-text "aisle") "aabbccddee" 0 4 2 10)))

(test-assert (= 1 (textual-prefix-length (as-text "bail") (as-text "aabbccddee") 0 1 2 10)))

(test-assert (= 0 (textual-prefix-length (as-text "prefix") (as-text "preface") 0 5 1 6)))

(test-assert (= 0 (textual-prefix-length (as-text "aisle") (as-text "") 1 4 0 0)))

(test-assert (= 0 (textual-prefix-length (as-text "aisle") (as-text "aabbccddee") 1 4 3 3)))

(test-assert (= 0 (textual-prefix-length (as-text "bail") (as-text "aabbccddee") 1 4 3 6)))

(test-assert (= 3 (textual-prefix-length (as-text "prefix") (as-text "preface") 1 5 1 7)))


(test-assert (= 0 (textual-suffix-length ABC ABCDEF)))

(test-assert (= 0 (textual-suffix-length ABCDEF ABC)))

(test-assert (= 0 (textual-suffix-length ABCDEF DEFABC)))

(test-assert (= 6 (textual-suffix-length DEFABC DEFABC)))

(test-assert (= 6 (textual-suffix-length (textual->string DEFABC) DEFABC)))

(test-assert (= 6 (textual-suffix-length DEFABC (textual->string DEFABC))))

(test-assert (= 6 (textual-suffix-length (textual->string DEFABC) (textual->string DEFABC))))

(test-assert (= 0 (textual-suffix-length (as-text "") (as-text ""))))

(test-assert (= 0 (textual-suffix-length (as-text "") (as-text "aabbccddee"))))

(test-assert (= 0 (textual-suffix-length (as-text "aisle") (as-text ""))))

(test-assert (= 0 (textual-suffix-length (as-text "") (as-text "aabbccddee"))))

(test-assert (= 1 (textual-suffix-length (as-text "aisle") (as-text "aabbccddee"))))

(test-assert (= 0 (textual-suffix-length (as-text "bail") (as-text "aabbccddee"))))

(test-assert (= 3 (textual-suffix-length (as-text "place") (as-text "preface"))))

(test-assert (= 0 (textual-suffix-length (as-text "") (as-text "") 0)))

(test-assert (= 0 (textual-suffix-length (as-text "") (as-text "aabbccddee") 0)))

(test-assert (= 0 (textual-suffix-length (as-text "aisle") (as-text "") 0)))

(test-assert (= 1 (textual-suffix-length (as-text "aisle") (as-text "aabbccddee") 0)))

(test-assert (= 0 (textual-suffix-length (as-text "bail") (as-text "aabbccddee") 0)))

(test-assert (= 3 (textual-suffix-length (as-text "place") (as-text "preface") 0)))

(test-assert (= 0 (textual-suffix-length (as-text "aisle") (as-text "") 1)))

(test-assert (= 1 (textual-suffix-length (as-text "aisle") (as-text "aabbccddee") 1)))

(test-assert (= 0 (textual-suffix-length (as-text "bail") (as-text "aabbccddee") 1)))

(test-assert (= 3 (textual-suffix-length (as-text "place") (as-text "preface") 1)))

(test-assert (= 0 (textual-suffix-length (as-text "") (as-text "") 0 0)))

(test-assert (= 0 (textual-suffix-length (as-text "") (as-text "aabbccddee") 0 0)))

(test-assert (= 0 (textual-suffix-length (as-text "aisle") (as-text "") 0 4)))

(test-assert (= 0 (textual-suffix-length (as-text "aisle") (as-text "aabbccddee") 0 4)))

(test-assert (= 0 (textual-suffix-length (as-text "bail") (as-text "aabbccddee") 0 1)))

(test-assert (= 0 (textual-suffix-length (as-text "aisle") (as-text "") 1 4)))

(test-assert (= 0 (textual-suffix-length (as-text "aisle") (as-text "aabbccddee") 1 4)))

(test-assert (= 1 (textual-suffix-length (as-text "aisle") (as-text "aabbccddee") 1 5)))

(test-assert (= 0 (textual-suffix-length (as-text "bail") (as-text "aabbccddee") 1 4)))

(test-assert (= 3 (textual-suffix-length (as-text "place") (as-text "preface") 1 5)))

(test-assert (= 0 (textual-suffix-length (as-text "") (as-text "") 0 0 0)))

(test-assert (= 0 (textual-suffix-length (as-text "") (as-text "aabbccddee") 0 0 0)))

(test-assert (= 0 (textual-suffix-length (as-text "aisle") (as-text "") 0 4 0)))

(test-assert (= 0 (textual-suffix-length (as-text "aisle") (as-text "aabbccddee") 0 4 2)))

(test-assert (= 0 (textual-suffix-length (as-text "bail") (as-text "aabbccddee") 0 1 2)))

(test-assert (= 3 (textual-suffix-length (as-text "place") (as-text "preface") 0 5 1)))

(test-assert (= 0 (textual-suffix-length (as-text "aisle") (as-text "") 1 4 0)))

(test-assert (= 0 (textual-suffix-length (as-text "aisle") (as-text "aabbccddee") 1 4 3)))

(test-assert (= 0 (textual-suffix-length (as-text "bail") (as-text "aabbccddee") 1 4 3)))

(test-assert (= 3 (textual-suffix-length (as-text "place") (as-text "preface") 1 5 1)))

(test-assert (= 0 (textual-suffix-length (as-text "") (as-text "") 0 0 0 0)))

(test-assert (= 0 (textual-suffix-length (as-text "") (as-text "aabbccddee") 0 0 0 0)))

(test-assert (= 0 (textual-suffix-length (as-text "aisle") (as-text "") 0 4 0 0)))

(test-assert (= 1 (textual-suffix-length "aisle" (as-text "aabbccddee") 0 5 2 10)))

(test-assert (= 1 (textual-suffix-length (as-text "bail") (as-text "aabbccddee") 0 1 2 4)))

(test-assert (= 0 (textual-suffix-length (as-text "place") (as-text "preface") 0 5 1 6)))

(test-assert (= 2 (textual-suffix-length (as-text "place") (as-text "preface") 0 4 1 6)))

(test-assert (= 0 (textual-suffix-length (as-text "aisle") (as-text "") 1 4 0 0)))

(test-assert (= 0 (textual-suffix-length (as-text "aisle") (as-text "aabbccddee") 1 4 3 3)))

(test-assert (= 0 (textual-suffix-length (as-text "bail") (as-text "aabbccddee") 1 4 3 6)))

(test-assert (= 3 (textual-suffix-length (as-text "place") (as-text "preface") 1 5 1 7)))
...
(test-assert (result=? "1234STRIKES" (textual-upcase (as-text "1234Strikes"))))

(test-assert (result=? "1234STRIKES" (textual-upcase (as-text "1234strikes"))))

(test-assert (result=? "1234STRIKES" (textual-upcase (as-text "1234STRIKES"))))

(test-assert (result=? "1234strikes" (textual-downcase (as-text "1234Strikes"))))

(test-assert (result=? "1234strikes" (textual-downcase (as-text "1234strikes"))))

(test-assert (result=? "1234strikes" (textual-downcase (as-text "1234STRIKES"))))

(test-assert (result=? "1234strikes" (textual-foldcase (as-text "1234Strikes"))))

(test-assert (result=? "1234strikes" (textual-foldcase (as-text "1234strikes"))))

(test-assert (result=? "1234strikes" (textual-foldcase (as-text "1234STRIKES"))))

(test-assert (result=? "And With Three Strikes You Are Out"
                       (textual-titlecase
...
(let* ((alphabet "abcdefghijklmnopqrstuvwxyz")
  ...
  (test-assert (result=? (string-append str1 str600-999)
                         (textual-concatenate (list alph1 t600-999))))

  (test-assert (result=? (string-append str1 str600-999)
                         (textual-concatenate (list alph1 (textual-copy t600-999)))))

  (test-assert (result=? (string-append str600-999 str1)
...
(test-assert (result=? "xyzabxyzcdxyzxyzexyzfxyz"
...
(test-assert (result=? "xyzabxyzcdxyzxyzexyzfxyzxyz"
...
(test-assert (result=? "xyzxyzabxyzcdxyzxyzexyzfxyz"
...
(test-assert (result=? "aiueaaaoi"
                       (textual-filter (lambda (c) (memv c (textual->list "aeiou")))
                                       (as-text "What is number, that man may know it?"))))

(test-assert (result=? "And wmn, tht sh my knw nmbr?"
                       (textual-remove (lambda (c) (memv c (textual->list "aeiou")))
                                       "And woman, that she may know number?")))

(test-assert (result=? "iueaaaoi"
                       (textual-filter (lambda (c) (memv c (textual->list "aeiou")))
                                       (as-text "What is number, that man may know it?")
                                       4)))

(test-assert (result=? "mn, tht sh my knw nmbr?"
                       (textual-remove (lambda (c) (memv c (textual->list "aeiou")))
                                       "And woman, that she may know number?"
                                       6)))

(test-assert (result=? "aaao"
                       (textual-filter (lambda (c) (memv c (textual->list "aeiou")))
                                       (as-text "What is number, that man may know it?")
                                       16 32)))

(test-assert (result=? "And woman, that sh may know"
...
(test-assert (result=? "cdefabcdefabcd"
                       (textual-replicate "abcdef" -4 10)))

(test-assert (result=? "bcdefbcdefbcd"
                       (textual-replicate "abcdef" 90 103 1)))

(test-assert (result=? "ecdecdecde"
