;; Tests from SRFI 14 reference implementation
;; Packaged for R7RS Scheme and SRFI 64
...
(test-assert (char-set= (string->char-set "eiaou2468013579999")
                        (char-set-unfold null? car cdr '(#\a #\e #\i #\o #\u #\u #\u)
                                         char-set:digit)))

(test-assert (char-set= (string->char-set "eiaou246801357999")
                        (char-set-unfold! null? car cdr '(#\a #\e #\i #\o #\u)
                                          (string->char-set "0123456789"))))

(test-assert (not (char-set= (string->char-set "eiaou246801357")
                             (char-set-unfold! null? car cdr '(#\a #\e #\i #\o #\u)
                                               (string->char-set "0123456789")))))

(test-assert (let ((cs (string->char-set "0123456789")))
               (char-set-for-each (lambda (c) (set! cs (char-set-delete cs c)))
                                  (string->char-set "02468000"))
               (char-set= cs (string->char-set "97531"))))

(test-assert (not (let ((cs (string->char-set "0123456789")))
                    (char-set-for-each (lambda (c) (set! cs (char-set-delete cs c)))
                                       (string->char-set "02468"))
                    (char-set= cs (string->char-set "7531")))))

(test-assert (char-set= (char-set-map char-upcase (string->char-set "aeiou"))
                        (string->char-set "IOUAEEEE")))

(test-assert (not (char-set= (char-set-map char-upcase (string->char-set "aeiou"))
                             (string->char-set "OUAEEEE"))))

(test-assert (char-set= (char-set-copy (string->char-set "aeiou"))
                        (string->char-set "aeiou")))

(test-assert (char-set= (char-set #\x #\y) (string->char-set "xy")))
(test-assert (not (char-set= (char-set #\x #\y #\z) (string->char-set "xy"))))

(test-assert (char-set= (string->char-set "xy") (list->char-set '(#\x #\y))))
(test-assert (not (char-set= (string->char-set "axy") (list->char-set '(#\x #\y)))))

(test-assert (char-set= (string->char-set "xy12345")
                        (list->char-set '(#\x #\y) (string->char-set "12345"))))
(test-assert (not (char-set= (string->char-set "y12345")
                             (list->char-set '(#\x #\y) (string->char-set "12345")))))
...
(test-assert (char-set= (string->char-set "aeiou12345")
                        (char-set-filter vowel? char-set:ascii (string->char-set "12345"))))
(test-assert (not (char-set= (string->char-set "aeou12345")
                             (char-set-filter vowel? char-set:ascii (string->char-set "12345")))))
...
(test-assert (char-set= (string->char-set "abcdef12345")
                        (ucs-range->char-set 97 103 #t (string->char-set "12345"))))
(test-assert (not (char-set= (string->char-set "abcef12345")
                             (ucs-range->char-set 97 103 #t (string->char-set "12345")))))
...
(test-assert (= 10 (char-set-size (char-set-intersection char-set:ascii char-set:digit))))
...
(test-assert (char-set= (->char-set "ABCD")
...
(test-assert (char-set= (char-set-intersection char-set:hex-digit (char-set-complement char-set:digit))
                        (->char-set "abcdefABCDEF")))
(test-assert (char-set= (char-set-intersection! (char-set-complement! (->char-set "0123456789"))
                                                char-set:hex-digit)
                        (->char-set "abcdefABCDEF")))
...
(test-assert (call-with-values (lambda ()
