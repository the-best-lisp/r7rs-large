...
(define-library
  ...
  (begin
    ...
    (define (check-arg pred val caller) (if (not (pred val)) (error val caller) val))
    ...
    ;; Reference implementation fails on Chibi (related to string=?)
    ;; so redefined to use operations from charset library itself
    (define (char-set= . rest)
      (define (pair-equal? cs1 cs2)
        (and (= (char-set-size cs1) (char-set-size cs2))
             (char-set-every (lambda (c) (char-set-contains? cs2 c))
                             cs1)
             (char-set-every (lambda (c) (char-set-contains? cs1 c))
                             cs2)))
      ;
      (if (null? rest)
        #t
    ...
    (define (char-set-hash cs . maybe-bound)
      (let* ((bound (if (null? maybe-bound) 
                      4194304 
                      (car maybe-bound)))	; 0 means default.
    ...
    (define (ucs-range->char-set lower upper . rest-in)
      (let* ((error? (if (> (length rest-in) 0) (car rest-in) #f))
             (rest (if (> (length rest-in) 0) (cdr rest-in) rest-in)))
    ...
    (define (%char-set-filter! pred ds bs proc)
      (check-arg procedure? pred %char-set-filter!)
      (let lp ((i 255))
        (when (>= i 0)
          (when (and (si=1? ds i) (pred (integer->char i)))
            (%set1! bs i))
          (lp (- i 1)))))
